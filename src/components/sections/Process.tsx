...
export const Process: React.FC = () => {
  ...
  return (
    <section id="process" className="py-20 sm:py-28 bg-[var(--color-bg)] relative overflow-hidden border-t border-[var(--color-border)]">
      <BackgroundDecoration variant="section" />

      {/* Background glow */}
      <div className="absolute top-1/2 left-0 w-[300px] sm:w-[450px] h-[300px] sm:h-[450px] bg-[var(--color-brand)] opacity-10 dark:opacity-20 rounded-full blur-[100px] sm:blur-[140px] pointer-events-none" />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-5 sm:gap-6 relative">
          ...
          <div className="hidden md:block absolute top-1/2 left-8 right-8 h-[2px] bg-[var(--color-border)] -translate-y-6 z-0" />
          <motion.div
            className="hidden md:block absolute top-1/2 left-8 right-8 h-[2px] bg-gradient-to-r from-transparent via-[var(--color-brand)] to-transparent -translate-y-6 z-0"
            animate={{ x: ["-100%", "100%"] }}
          ...
          {PROCESS_STEPS.map((step, idx) => {
            ...
            return (
              <motion.div
                ...
                className="relative z-10 p-5 sm:p-6 rounded-2xl sm:rounded-3xl bg-[var(--color-bg-card)] border border-[var(--color-border)] transition-all duration-300 group flex flex-col justify-between overflow-hidden"
                style={{
                  boxShadow: isHovered
              ...
              >
                <div>
                  {/* Number Badge */}
                  <div className="w-10 sm:w-12 h-10 sm:h-12 rounded-xl sm:rounded-2xl bg-[var(--color-bg-card-elevated)] border border-[var(--color-border)] text-[var(--color-brand)] font-mono font-bold text-base sm:text-lg flex items-center justify-center mb-4 sm:mb-6 group-hover:bg-[var(--color-brand)] group-hover:text-white transition-all duration-300 shadow-md">
                    {step.number}
                  </div>

                  <h3 className="text-lg sm:text-xl font-bold text-[var(--color-fg)] mb-1.5 sm:mb-2 group-hover:text-[var(--color-brand)] transition-colors duration-200">
                    {step.title}
                  </h3>
